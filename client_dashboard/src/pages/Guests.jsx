    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6 flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{data.total} guests captured from calls</p>
        </div>
        <input
            ...
            <tbody>
              ...
              {data.data.map(g => (
                <tr key={g.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-3.5 font-medium text-gray-800">{g.name || '—'}</td>
                  <td className="px-6 py-3.5 text-gray-600">{g.phone || '—'}</td>
                  <td className="px-6 py-3.5 text-gray-400 text-xs">{g.created_at ? new Date(g.created_at).toLocaleDateString() : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
