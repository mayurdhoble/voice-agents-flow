      ...
      <div className="relative w-full max-w-lg bg-white h-full shadow-xl flex flex-col">
        ...
        {call.recording_url && (
          <div className="px-6 py-4 border-b border-gray-100">
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Recording</p>
            <audio controls src={call.recording_url} className="w-full" />
          </div>
        )}

        <div className="flex-1 overflow-y-auto px-6 py-4">
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-3">Conversation</p>
              ...
              return (
                <div key={i} className={`flex ${isGuest ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-3.5 py-2 text-sm ${
                    isGuest ? 'bg-gray-100 text-gray-800 rounded-tl-sm'
                            : 'bg-indigo-600 text-white rounded-tr-sm'}`}>
                    <p className={`text-[10px] font-semibold uppercase tracking-wide mb-0.5 ${isGuest ? 'text-gray-400' : 'text-indigo-200'}`}>
                      {isGuest ? 'Guest' : 'Maya'}
                    </p>
                    {m.content}
                  </div>
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6 flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{data.total} calls · click a row to hear the recording &amp; read the transcript</p>
        </div>
        <input
          value={search} onChange={e => setSearch(e.target.value)} placeholder="Search phone…"
          className="border border-gray-300 rounded-lg px-3 py-2 text-sm w-56 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
        />
      </div>
                  ...
                  <td className="px-6 py-3.5">
                    {c.recording_url ? (
                      <span className="inline-flex items-center gap-1.5 text-emerald-600 text-xs font-medium">
                        <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-gray-800 shadow-sm">
