  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">{data.total} party / function enquiries for your events team to follow up</p>
      </div>
            ...
            <tbody>
              ...
              {data.data.map(e => (
                <tr key={e.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-3.5 text-gray-700 capitalize">{e.event_type || '—'}</td>
                  <td className="px-6 py-3.5 text-gray-500 text-xs">{e.event_date || '—'}</td>
                  <td className="px-6 py-3.5 text-gray-500 tabular-nums">{e.num_guests ?? '—'}</td>
                  <td className="px-6 py-3.5">
                    <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-violet-50 text-violet-700 capitalize">
                      {e.status || 'inquiry'}
                    </span>
                  </td>
                  <td className="px-6 py-3.5 text-gray-400 text-xs">{e.created_at ? new Date(e.created_at).toLocaleDateString() : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
